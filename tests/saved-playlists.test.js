...
it('saves a queue snapshot once and refreshes persisted entries',async()=>{
 ...
 const card={_state:{menuPage:'saved_playlists'},_m:a=>a,_esc:String,_getNowPlayingQueueItems:()=>[{uri:'spotify://track/a'}],_getQueueItemUri:item=>item.uri,_mediaControlFailureMessage:String,_maverickEngineCommand:vi.fn(async(_,args)=>args.action==='list'?[]:{id:'one'})};
 ...
 await vi.waitFor(()=>expect(card._maverickEngineCommand).toHaveBeenCalledTimes(3));
 expect(card._maverickEngineCommand.mock.calls.filter(([,args])=>args.action==='save')).toEqual([['playlists',{action:'save',name:'My mix',uris:['spotify://track/a']}]]);body.remove();
});
it('ignores a playlist response when the user leaves the page',async()=>{
 ...
 const card={_state:{menuPage:'saved_playlists'},_m:a=>a,_esc:String,_maverickEngineCommand:()=>new Promise(resolve=>finish=resolve)};
...
it('does not replace the next screen after an in-flight save completes',async()=>{
 const body=document.createElement('div');document.body.append(body);let finish;
 const card={_state:{menuPage:'saved_playlists'},_m:a=>a,_esc:String,_getNowPlayingQueueItems:()=>[{uri:'library://track/1'}],_getQueueItemUri:item=>item.uri,_mediaControlFailureMessage:String,_maverickEngineCommand:vi.fn(async()=>[])};
 await renderSavedPlaylists(card,body);body.querySelector('input').value='Mix';
 card._maverickEngineCommand.mockImplementation(()=>new Promise(resolve=>finish=resolve));
 ...
 expect(body.textContent).toBe('Players');expect(card._maverickEngineCommand).toHaveBeenCalledTimes(2);body.remove();
});
