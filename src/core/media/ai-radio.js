...
export async function renderAiRadio(card, body) {
  ...
  body.innerHTML = `<div class="lyrics-state">${t("Loading AI Radio…")}</div>`;
  try {
    if (!queueId) throw new Error(card._m("Select a player with an active queue first."));
    ...
    body.innerHTML = `<div class="announcements-shell ai-radio-shell"><h3>${t("Your queue, with a radio host")}</h3><p>${t("Choose a host configured in Music Assistant. MA generates the spoken segments using its AI and voice settings.")}</p><label class="announcement-target">${t("Host")}<select class="settings-select" data-ai-host><option value="">${t("DJ off")}</option>${available.map(host => `<option value="${card._esc(host.id)}" ${host.id === current ? "selected" : ""}>${card._esc(host.name)}</option>`).join("")}</select></label>${!available.length ? `<p>${t("Configure an AI Radio host in MA first.")}</p>` : ""}<button class="action-btn" data-ai-apply>${t("Apply to this queue")}</button><p role="status" data-ai-status></p></div>`;
    body.querySelector("[data-ai-apply]").addEventListener("click", async event => {
      ...
      try {
        ...
        if ((confirmed?.[queueId] || null) !== host) throw new Error(card._m("MA has not confirmed the DJ change."));
        feedback.textContent = card._m("DJ setting confirmed by MA.");
