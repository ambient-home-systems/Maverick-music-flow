...
const groups = [
  ["Autoplay", "radio", ["autoplay_enabled", "autoplay_mode", "autoplay_playlist"]],
  ["Smart Shuffle", "shuffle", ["smart_shuffle_enabled", "smart_shuffle_optimize_smart_fades"]],
  ["Transitions", "crossfade", ["crossfade_enabled", "crossfade_mode", "crossfade_duration"]],
];
const labels = {
  autoplay_enabled: "Autoplay by default",
  autoplay_mode: "Continue playing from",
  autoplay_playlist: "Playlist",
  smart_shuffle_enabled: "Use listening history when shuffling",
  smart_shuffle_optimize_smart_fades: "Optimize track order for Smart Fades",
  crossfade_enabled: "Crossfade by default",
  crossfade_mode: "Transition style",
  crossfade_duration: "Standard transition duration (seconds)",
};
const optionLabels = {
  enabled: "Enabled", disabled: "Disabled",
  auto: "Similar music, then your library",
  similar: "Similar music", library: "Your library",
  playlist: "A selected playlist",
  standard_crossfade: "Standard crossfade",
...
export function queueSettingsHtml(card, view) {
  ...
  const field = (key) => {
    ...
    if (entry.type === "boolean") {
      ...
      const options = key === "autoplay_playlist"
        ? [{ value: "", title: card._m("Choose a playlist") }, ...playlists.map((item) => ({ value: item.uri, title: item.name }))]
        : entry.options;
      if (entry.value && !options.some((option) => option.value === entry.value)) options.push({ value: entry.value, title: card._m("Current playlist") });
  ...
  return `<form class="queue-settings-form">
    <p class="action-hub-note">${esc(card._m("Shared Music Assistant preferences for all players. Per-queue Autoplay and Crossfade buttons remain independent."))}</p>
    ${!editable ? `<p role="status">${esc(card._m("An administrator can edit these preferences."))}</p>` : ""}
    ${view.playlistsUnavailable ? `<div class="notice open" role="status">${esc(card._m("Playlists could not be loaded. Your current playlist is preserved; other preferences can still be edited."))}<button type="button" class="action-btn" data-menu-action="reload_queue_settings">${esc(card._m("Reload playlists"))}</button></div>` : ""}
    <fieldset ${editable ? "" : "disabled"}>${groups.map(([en, icon, keys]) => keys.some((key) => entries[key]) ? `<section class="action-hub-section"><h3>${card._iconSvg(icon)}${esc(card._m(en))}</h3><div class="queue-settings-fields">${keys.map(field).join("")}</div></section>` : "").join("")}</fieldset>
    <div class="queue-settings-footer"><button type="button" class="action-btn" data-menu-action="save_queue_settings" ${editable ? "" : "disabled"}>${card._iconSvg("check")}${esc(card._m("Save changes"))}</button><span role="status" data-queue-settings-status></span></div>
  </form>`;
}
...
export async function loadQueueSettings(card, body, isCurrent) {
  body.innerHTML = `<div class="notice open" role="status">${card._esc(card._m("Loading playback preferences…"))}</div>`;
  try {
    ...
    if (!settings.value?.entries) throw new Error(card._m("Playback preferences were not returned."));
    ...
    if (isCurrent()) body.innerHTML = `<div class="notice open" role="alert">${card._esc(card._mediaControlFailureMessage(error))}</div><button class="action-btn" data-menu-action="reload_queue_settings">${card._esc(card._m("Retry"))}</button>`;
  }
}
...
export async function saveQueueSettings(card) {
  ...
  try {
    ...
    if (!result?.saved || Object.entries(values).some(([key, value]) => result.entries?.[key]?.value !== value)) throw new Error(card._m("The save was not confirmed. Reload to check the current values."));
    view.entries = result.entries;
    status.textContent = card._m("Saved in Music Assistant");
  } catch (error) {
