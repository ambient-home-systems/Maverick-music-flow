...
export function getMobileEditorTexts() {
  ...
  return {
    ...
    labels: {
      ...
      pinned_player_master: HomeiiEditorLocale.detectEditorHebrew() ? "נגן MASTER" : "MASTER player",
      entity_sticky: HomeiiEditorLocale.detectEditorHebrew() ? "חזור ל־MASTER לאחר חוסר פעילות" : "Return to MASTER after inactivity",
      pinned_players_exclusive: HomeiiEditorLocale.detectEditorHebrew() ? "מצב Exclusive" : "Exclusive mode",
    ...
    helpers: {
      ...
      pinned_player_master: HomeiiEditorLocale.detectEditorHebrew() ? "בחר נגן ראשי אחד מתוך הנגנים המוצמדים. ה־Sticky תמיד חוזר אליו." : "Choose one primary player from the pinned players. Sticky always returns to it.",
      entity_sticky: HomeiiEditorLocale.detectEditorHebrew() ? "מאפשר לבחור ולעבוד זמנית עם כל נגן זמין, ולאחר 10 שניות ללא פעולה חוזר ל־MASTER." : "Allows temporary access to any available player, then returns to MASTER after 10 idle seconds.",
      pinned_players_exclusive: HomeiiEditorLocale.detectEditorHebrew() ? "כאשר מופעל, רק נגני PINNED מוצגים. כאשר כבוי, כל הנגנים הזמינים נשארים נגישים." : "When enabled, only PINNED players are shown. When disabled, every available player remains accessible.",
